...
Write-Host "Can't give correct result with POSCARs having off-diagonal elements. Only Cubic and Tetragonal POSCARs can be merged accurately." -ForegroundColor Red
...
$($data1[0].Trim()+'/'+$data2[0].Trim())
 $("{0:n10}" -f ($lc1*$x1[0]))
         ...
         0.000000000         $("{0:n9}" -f ($y1[1]/$x1[0]))         0.000000000
         0.000000000         0.000000000         $("{0:n9}" -f ($totalZ/($lc1*$x1[0])))
    $Elements
    $total_slab
Direct
...
$value_new=([float]$value[2])*$factor1
 $value[0]+'       '+$value[1]+'      '+$("{0:n9}" -f ($value_new))|Add-Content $outfile
}
