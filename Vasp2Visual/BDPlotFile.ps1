...
$SimplePlot=@'
K=KE[:,3]; E=KE[:,4:]; #Seperate KPOINTS and Eigenvalues in memory
D=data_dos[:,0];TDOS=data_dos[:,1]; eGrid_DOS=int(np.shape(TDOS)[0]/ISPIN); #Energy grid mesh
yh=max(E_Limit);yl=min(E_Limit);        
#==================================================================
max_E=np.max(np.where(E[0,:]-E_Fermi <=yh)); min_E=np.min(np.where(E[0,:]-E_Fermi >=yl))
max_index=np.max(np.where(data_dos[:eGrid_DOS,0]-E_Fermi <=yh)); min_index=np.min(np.where(data_dos[:eGrid_DOS,0]-E_Fermi >=yl))
E=E[:,min_E:max_E]-E_Fermi; D=D[min_index:max_index]-E_Fermi  #Updated energy in E_limit and set reference to Fermi Energy
TDOS=TDOS[min_index:max_index]; #Updated total DOS
#=================Plotting============================
wd=WidthToColumnRatio; #sets width w.r.t atricle's column width
