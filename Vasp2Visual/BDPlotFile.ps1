...
"@  #Files Loaded
$CollectData=@'
K=KE[:,3]; E=KE[:,4:]; #Seperate KPOINTS and Eigenvalues in memory
D=pdos[:,0];TDOS=data_dos[:,1]; eGrid_DOS=int(np.shape(TDOS)[0]/ISPIN); #Energy grid mesh
...
max_E=np.max(np.where(E[0,:]-E_Fermi <=yh)); min_E=np.min(np.where(E[0,:]-E_Fermi >=yl))
max_index=np.max(np.where(pdos[:eGrid_DOS,0]-E_Fermi <=yh)); min_index=np.min(np.where(pdos[:eGrid_DOS,0]-E_Fermi >=yl))
...
red=red[:,min_E:max_E]/max_con;green=green[:,min_E:max_E]/max_con;blue=blue[:,min_E:max_E]/max_con #Values are ready in E_Limit
red_dos=red_dos[min_index:max_index];green_dos=green_dos[min_index:max_index]; blue_dos=blue_dos[min_index:max_index]; #DOS in E_Limit
Elem_dos=red_dos+blue_dos+green_dos; #ION DOS in E_Limit
E=E[:,min_E:max_E]-E_Fermi; D=D[min_index:max_index]-E_Fermi  #Updated energy in E_limit and set reference to Fermi Energy
TDOS=TDOS[min_index:max_index]; #Updated total DOS
#===============Make Collections======================
...
plt.figure(figsize=(wd*3.4,wd*2.5))
...
ax_settings(ax1,ticks,ticklabels,0.05,0.9,ProLabels[0])
...
#================StackPlots for DOS====================================
ax2.fill_between(TDOS,pdos[max_index,0],D,color=(167/300, 216/300, 222/300),facecolor=(1, 1, 1),linewidth=0);
ax2.fill_between(blue_dos+green_dos+red_dos,pdos[max_index,0],D,color=(0,0,1),linewidth=0); 
ax2.fill_between(green_dos+red_dos,pdos[max_index,0],D,color=(0,1,0),linewidth=0);
ax2.fill_between(red_dos,pdos[max_index,0],D,color=(1,0,0),linewidth=0);
ax2.set_ylim([yl,yh]);ax2.set_xlim([min(TDOS),max(TDOS)/16]);
...
#===================Name it & Save===============================
ProLabels=[prolabel.replace("$","").replace("_","") for prolabel in ProLabels]; #Remove $ and _ characters in path
...
name=str(SYSTEM+'_'+ProLabels[0]+'['+atom_index_range+']'+'('+str(ProLabels[1])+')('+str(ProLabels[2])+')('+str(ProLabels[3])+')'+'_A'); #A for All,B for Bnads, D for DOS,S for Simple
plt.savefig(str(name+'.svg'),transparent=True)
plt.savefig(str(name+'.pdf'),transparent=True)
plt.show()
'@
#Making a full dynamic file string for python use
$FileString=@"
$SystemVariables
$ImportPackages
$LoadFiles
