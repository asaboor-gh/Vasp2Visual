...
Write-Host @"
 $sys structure contains  $NION ions and $NBANDS bands.          
...
"@  -NoNewline -ForegroundColor Green #Don't change it
...
Write-Host @"
 Done âœ”: $([Math]::Round($($timer.Elapsed.TotalSeconds),3)) seconds elapsed.
...
Write-Host "â–¼ " -ForegroundColor Cyan -NoNewline
Write-Host @"
 SYSTEM: $sys, NIONS: $NION, NBANDS: $NBANDS, Filled: $filled, NKPTS: $($NKPTS-$ibzkpt) 
