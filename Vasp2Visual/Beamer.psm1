...
\bNode{\begin{tabular}{ p{2cm}p{1cm}p{1cm}  }
		%\toprule 
...
{
	\bNode{Progress bar also shows stopwatch over a predefined presentation time by setting \alert{$\backslash$pgfmathsetmacro\{$\backslash$TotalTime\}\{Minutes\}} in permeable of document. Time is shown only in Adobe Reader. The package \alert{tdclock} from CTAN is used to fetch system time in document.}
}
'@
$end=@'
%-----------------------------------------------------------------------
...
\begin{center}
	\begin{tikzpicture}
	\draw[] node[align=center, text width=0.2 \textwidth ,inner sep=10pt,opacity=0.6] {\rNode{{\color{blue!50!green} \bf \LARGE THANKS }}};
	\end{tikzpicture}
\end{center}
\end{frame}
}
